...
while True:
  ...
  duration_log.append(time.time() - start)
  ...
  for i in range(len(route_table.split('-'))):
    ...
    lend = time.time()
    duration_log.append(lend - lstart)

  end = time.time()
  print("duration = %s with <%.4f, %.4f, %.4f, %.4f>" % (end - start, duration_log[0], duration_log[1], duration_log[2], duration_log[3]))

  frame_id += 1

etime = time.time()
print("total fps = %f" % (float(frame_count) / (etime - stime)))