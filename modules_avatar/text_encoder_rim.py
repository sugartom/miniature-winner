...
class TextEncoder:
    ...
    def PreProcess(self, request, istub, grpc_flag):
        if (grpc_flag):
            self.request = str(tensor_util.MakeNdarray(request.inputs["speech_recognition_output"])).decode('utf-8')
        else:
            self.request = request["speech_recognition_output"]
        self.istub = istub

    def Apply(self):
        encoded_src_list = self.sp1.EncodeAsPieces(self.request)
