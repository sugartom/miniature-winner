...
class Transformer:
    ...
    def PreProcess(self, input, stub):
        self.stub = stub
        feed_dict = Transformer.model.get_data_layer().create_feed_dict(input)

        return feed_dict

    def Apply(self, feed_dict):
        src_text = feed_dict[Transformer.model.get_data_layer().input_tensors[
            "source_tensors"][0]]
        src_text_length = feed_dict[Transformer.model.get_data_layer().input_tensors[
