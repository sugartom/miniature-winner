...
def iterate_data(model, sess, compute_loss, mode, verbose, num_steps=None):
    ...
    for worker_id in range(model.num_gpus):
        ...
        if size_defined:
            dl_sizes.append(model.get_data_layer(
                worker_id).get_size_in_samples())
        try:
            total_objects = 0.0
    ...
    while True:
        ...
        for worker_id, fetches_val in fetches_vals.items():
            ...
            # assuming any element of inputs["source_tensors"] .shape[0] is
            # batch size
            batch_size = inputs["source_tensors"][0].shape[0]
            total_samples[worker_id] += batch_size

            if size_defined:
                # this data_layer is at the last batch with few more elements,
                # cutting
                if total_samples[worker_id] > dl_sizes[worker_id]:
                    last_batch_size = dl_sizes[worker_id] % batch_size
                    for key, value in inputs.items():
                        inputs[key] = model.clip_last_batch(
                            value, last_batch_size)
        ...
        if verbose:
            if size_defined:
                data_size = int(
                    np.sum(np.ceil(np.array(dl_sizes) / batch_size)))
...
def get_results_for_epoch(model, sess, compute_loss, mode, verbose=False):
    ...
    results_per_batch = collect_if_horovod(
        results_per_batch, model.hvd, 'gather')
        ...
        if isinstance(value, dict):
            if key in org_dict:
                if not isinstance(org_dict[key], dict):
                    raise ValueError(
                        "Mismatch between org_dict and upd_dict at node {}".format(
                            key)
                    )
...
def get_base_config(args):
    ...
    if base_model is None:
        raise ValueError(
            'base_config class has to be defined in the config file')
        ...
        if type(value) == int or type(value) == float or \
           isinstance(value, string_types):
            ...
            parser_unk.add_argument(
                '--' + pm, default=value, type=ast.literal_eval)
...
def check_logdir(args, base_config, restore_best_checkpoint=False):
    ...
    try:
        ...
        if args.mode == 'train' or args.mode == 'train_eval':
            ...
            if os.path.isdir(logdir) and os.listdir(logdir) != []:
                # if os.path.isdir(logdir) and 'checkpoint' in
                # os.listdir(logdir):
...
def create_model(args, base_config, config_module, base_model, hvd, restore_best_checkpoint=False):
    ...
    if args.mode == 'train' or args.mode == 'train_eval':
        if 'train_params' in config_module:
            nested_update(train_config, copy.deepcopy(
                config_module['train_params']))
        if hvd is None or hvd.rank() == 0:
            deco_print("Training config:")
            pprint.pprint(train_config)
    if args.mode == 'eval' or args.mode == 'train_eval':
        if 'eval_params' in config_module:
            nested_update(eval_config, copy.deepcopy(
                config_module['eval_params']))
        if hvd is None or hvd.rank() == 0:
    ...
    if args.mode == "infer":
        ...
        if "infer_params" in config_module:
            nested_update(infer_config, copy.deepcopy(
                config_module['infer_params']))
        if hvd is None or hvd.rank() == 0:
    ...
    if args.mode == 'train_eval':
        ...
        model.compile(checkpoint=checkpoint, use_trt=args.use_trt,
